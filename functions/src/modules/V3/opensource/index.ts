import { IChapterResponse, IUserInputsAndChaptersResponse, askExtraWords, promptForDialogueSystem, promptForEachChapter, promptForSystem, promptForUserInputsAndChapters } from './prompts/index';
...
export const openrouterApiKey = defineSecret("OpenRouter");
...
export async function GenerateStoryFromText(
    ...
    const dialogueParserAgent: ChatCompletionMessageParam[] = [
        {
            role: "system", content: promptForDialogueSystem(language)
        },
    ]
    ...
    info(`Chaperts: ${JSON.stringify(chaptersResponse.content)}`)
    ...
    while (chapterIndex < chapterAmount) {
        ...
        dialogueParserAgent.push({ role: "user", content: chapter })
        const dialogueParserResponse = await RequestChatGPT(dialogueParserAgent, "deepseek/deepseek-r1:free", 0)
        if (!dialogueParserResponse?.content) throw Error("Content is undefined")
        dialogueArray.push(dialogueParserResponse.content)
        dialogueParserAgent.pop()
    ...
    return { title, coverImagePrompt, story: mainStory, storyArray, dialogueArray }
}
