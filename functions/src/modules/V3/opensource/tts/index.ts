...
export async function GenerateBufferFromText({ text, voice, model }: IProps) {
    ...
    // Map over the textArray and use limit to control concurrency
    const bufferPromises = textArray.map((chunk) =>
        limit(async () => {
            const buffer = await TextToSpeech({ text: chunk, voice, model });
